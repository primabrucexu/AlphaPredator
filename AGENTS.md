...
新需求、新功能或用户可见行为变化必须严格依次经过以下阶段，不得合并或跳过：

1. 需求讨论：澄清目标、范围、非目标、关键取舍和待确认事项；此阶段不得修改文档或代码。
2. 文档编写：用户确认讨论结论后，才能新增或修改对应 Feature 文档及索引；完成后必须停止并向用户报告文档变更。
3. 文档确认：用户明确确认已经写入的文档后，才能进入实现准备；未确认时不得修改生产代码或测试代码。
4. 实现计划：根据已确认文档提交简短编码计划、影响文件和验收标准；完成后必须停止等待用户确认。
5. 编码实现：只有用户明确确认实现计划后，才能修改代码、测试及实现状态文档，并按验收标准完成验证。

用户对当前阶段回复“确认”，只授权进入紧邻的下一阶段，不得解释为同时授权后续多个阶段：

- 讨论后的“确认”只授权写入文档。
- 文档后的“确认”只授权准备实现计划。
- 实现计划后的“确认”才授权开始编码。

在用户明确要求“只讨论”时，不得修改任何文件；在用户明确要求“只写文档”时，完成文档后必须停止，不得继续实现。
...
11. 新增选股规则或新增规则 revision 时，必须同步提供与 `rule_id`、`rule_revision` 精确匹配并按规则版本拆分的机器可读规则摘要。摘要必须包含 `rule_metadata` 和 `summary_data`，字段及语义以 F008 为准；摘要不维护独立版本号，可直接修正或完善文字并由 Git 保留修改历史。规则实现完成但摘要缺失、格式无效或版本不匹配时，不得将该规则版本视为完成。规则文档阶段先确认摘要内容，运行时摘要 JSON 在编码实现阶段一并新增；固定参数实际值仍由任务记录提供，报告不得解析 Markdown 获取摘要。仅修改摘要表述且不改变规则语义时不需要新增规则 revision；业务语义变化必须新增 revision 和对应摘要文件。
...
- 同时影响前后端时，两项都必须执行。
- 新增选股规则或规则 revision 时，必须验证摘要 JSON 结构有效、元数据与注册规则一致，并确认所有需要报告支持的规则版本均存在对应摘要。
