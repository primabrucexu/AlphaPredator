import { BarChartOutlined, CheckOutlined, CloseOutlined, DeleteOutlined, DragOutlined, EditOutlined, FundOutlined, PlusOutlined, ScheduleOutlined, SearchOutlined, StarOutlined } from '@ant-design/icons'
...
export default function TagSidebar() {
  ...
  const activeTasks = useQuery({ queryKey: ['active-task-count'], queryFn: () => api.activeTaskCount('EXCLUSIVE_UPDATE'), refetchInterval: 3000 })
  const activeScreeningTasks = useQuery({ queryKey: ['active-screening-task-count'], queryFn: () => api.activeTaskCount('COMPUTE'), refetchInterval: 3000 })
  ...
  const navKey = location.pathname.startsWith('/screening') ? '/screening' : location.pathname.startsWith('/tasks') ? '/tasks' : '/'
  return <Fragment>
    <Layout.Sider width={200} theme="light" className="sidebar primary-sidebar">
      ...
      <Menu mode="inline" selectedKeys={[navKey]} items={[
        ...
        { key: '/screening', icon: <FundOutlined />, label: <Link to="/screening">模式选股 <Badge count={activeScreeningTasks.data?.count ?? 0} size="small" /></Link> },
