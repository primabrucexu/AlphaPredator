...
以下 DBML 是设计方案，不能直接落表。必须先经用户审批，并由用户更新 `docs/human/data-model/AlphaPredator.dbml` 后，才能修改 SQLite / DuckDB schema。
...
Table macd_alert_result [headercolor: #7a3f8f] {
  ...
  score numeric [not null, default: 0, note: '排序分，用于前端默认排序；可由临界价距离、绿柱缩短幅度等计算。']
...
`scan_macd_alerts`、`track_macd_alerts` 和 `get_macd_alert_daily_brief` 默认不生成报告文件，只返回 `text_summary` 和 `report_available=true`。
...
- 需要用户审批数据库设计方案。
- 审批后需要用户将表设计更新到 `docs/human/data-model/AlphaPredator.dbml`。
