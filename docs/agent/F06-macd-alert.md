...
2. 交易规则是否有效：T+1 开盘买入后，金叉前是否因趋势破坏且 3 个交易日内未修复而卖出；如果后续形成金叉，则按金叉后卖出规则计算收益。
```
...
如果同一只股票在连续多个交易日都满足 MACD 预警形态，则视为同一个连续命中区间，只用区间第一个交易日生成一条历史样本。区间内后续交易日不再重复生成回测样本，避免同一轮金叉趋势被重复买入、重复统计。
...
- T+1 趋势走弱：进入金叉前趋势修复观察；若 3 个交易日内未修复，则卖出。
...
如果买入后、首次金叉前出现 MACD 绿柱重新放大，视为金叉趋势被破坏，并从破坏日开始观察 3 个交易日。

金叉趋势修复定义：

```text
当日 macd_hist < 0
abs(macd_hist[当日]) < abs(macd_hist[前一交易日])
```
...
- 个股验证接口已支持将带空白、交易所前缀或交易所后缀的股票代码输入规范化为 6 位代码，避免常见输入格式在 schema 校验阶段返回 422。
- 个股验证前端已对齐首页股票搜索交互，支持代码、名称、拼音/首字母、交易所前后缀等输入并在选中后用 6 位股票代码联动接口。
- 个股验证截止日如为非交易日或本地无当日日线，会自动使用该日期之前最近一条日线作为实际验证日期，并在响应 `end_date` 中返回实际使用日期。
- 批量扫描接口已按日期联动规则修复：当用户选择周末、非交易日或本地当日日线缺失/明显不完整时，会按当前扫描股票池的日线覆盖度解析到此前最近有效交易日；后台任务 `start_date/end_date`、预警落库日期和前端结果查询日期统一使用实际扫描日。
