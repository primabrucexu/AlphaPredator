...
3. **数据表是否全部需要**：尤其是 `trade_review_attachment` 是否先跳过。
4. **附件存储 / OCR 方案**：本地路径、SQLite 元数据、还是后续接对象存储。
...
## 12. 用户决策记录（2026-05-28）

以下为用户对第 10 节"待人工决策事项"的最终决定，编码阶段必须严格遵守。

| # | 决策事项 | 决策结论 |
|---|---------|---------|
| 1 | 页面入口放哪里 | **侧边导航新增单个"交易复盘"菜单项**；页面内用 Tab 页区分"个股复盘"和"月度复盘"，不在侧边栏拆为多个入口 |
| 2 | AI 结果是否独立页面 | **内嵌**在详情页与月度页中，暂不单独建页面 |
| 3 | 数据表范围 | **除 `trade_review_attachment` 外，其余 5 张表全部通过**（含可选的 `trade_review_monthly_summary`）|
| 4 | 附件存储 / OCR 方案 | **前端上传图片后直接在内存中 OCR 识别**，识别结果返回前端展示，由用户手工核对修正；不持久化附件，不建 attachment 表 |
| 5 | 未清仓持仓是否参与复盘 | **是**，持仓中的股票可以创建复盘记录（`status=open`），清仓后更新为 `closed` |

### 对设计文档的影响

- **第 7.4 节（`trade_review_attachment` 表）**：取消，不实现
- **第 3 节 B 区块 5"原始材料"**：调整为"OCR 校对区域"，不做历史附件列表
- **第 3 节 C Step 2"上传交易图与 OCR 校对"**：交互不变，但后端只做图片→OCR→返回结果，不落库
- **导航结构**：侧边导航新增**单个"交易复盘"菜单项**，点击后进入复盘页面；页面内通过 **Tab 页**区分"个股复盘"和"月度复盘"两个功能区，不在侧边栏拆分为多个入口
